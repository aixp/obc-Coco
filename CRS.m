...
TYPE
	ErrorProc* = PROCEDURE (n: SHORTINT; pos: INTEGER);
	StartTable = ARRAY 128 OF SHORTINT;
  
VAR
	src*: Texts.Text;  (*source text. To be set by the main pgm*)
	pos*: INTEGER;  (*position of current symbol*)
	line*, col*, len*: SHORTINT;  (*line, column, length of current symbol*)
	nextPos*: INTEGER;  (*position of lookahead symbol*)
	nextLine*, nextCol*, nextLen*: SHORTINT;  (*line, column, length of lookahead symbol*)
	errors*: SHORTINT;  (*number of errors detected*)
	Error*: ErrorProc;

	ch: CHAR;        (*current input character*)
	r: Texts.Reader;	(*global reader*)
	chPos: INTEGER;	(*position of current character*)
	chLine: SHORTINT;  (*current line number*)
	lineStart: INTEGER;  (*start position of current line*)
	oldEols: SHORTINT;     (*nr. of EOLs in a comment*)
...
PROCEDURE Comment(): BOOLEAN;
	VAR level, startLine: SHORTINT; oldLineStart: INTEGER;
BEGIN (*Comment*)
...
VAR state: SHORTINT; lexeme: ARRAY maxLexLen+1 OF CHAR;
...
PROCEDURE GetName*(pos: INTEGER; len: SHORTINT; VAR s: ARRAY OF CHAR);
	VAR i: SHORTINT; r: Texts.Reader;
BEGIN
...
PROCEDURE StdErrorProc* (n: SHORTINT; pos: INTEGER);
BEGIN INC(errors) END StdErrorProc;

PROCEDURE Reset* (t: Texts.Text; pos: INTEGER; errProc: ErrorProc);
BEGIN
	src := t; Error := errProc;
	Texts.OpenReader(r, src, pos);
