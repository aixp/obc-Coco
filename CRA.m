...
IMPORT Oberon, Texts, Sets, CRS, CRT, Files:=OFiles;
...
TYPE
	...
	StateNode = RECORD         (*state of finite automaton*)
		nr: SHORTINT;  (*state number*)
		firstAction: Action;   (*to first action of this state*)
		endOf: SHORTINT;  (*nr. of recognized token if state is final*)
		ctx: BOOLEAN;  (*TRUE: state reached by contextTrans*)
		next: State
	END;
	ActionNode = RECORD    (*action of finite automaton*)
		typ: SHORTINT;       (*type of action symbol: char, class*)
		sym: SHORTINT;       (*action symbol*)
		tc: SHORTINT;       (*transition code: normTrans, contextTrans*)
...
VAR
	...
	lastSimState: SHORTINT;      (* last non melted state *)
	stateNr: SHORTINT;  (*number of last allocated state*)
...
PROCEDURE PutI2(i, n: SHORTINT);
BEGIN Texts.WriteInt(out, i, n) END PutI2;

PROCEDURE PutC(ch: CHAR);
BEGIN
	IF (ch < " ") OR (ORD(ch) = 34) THEN PutS("CHR("); PutI(SHORT(ORD(ch))); Put(")")
	ELSE Put(CHR(34)); Put(ch); Put(CHR(34))
	END
END PutC;

PROCEDURE PutRange(s: CRT.Set);
	VAR lo, hi: ARRAY 32 OF CHAR; top, i: SHORTINT; s1: CRT.Set;
BEGIN
	(*----- fill lo and hi *)
	top := -1; i := 0;
	WHILE i < 128 DO
...
BEGIN
	...
	WHILE a # NIL DO
		...
		ELSIF a.typ = CRT.class THEN
			...
			IF Sets.In(set, SHORT(ORD(ch))) THEN RETURN a END
		END;
		a := a.next
	END;
	RETURN NIL
...
PROCEDURE NewTransition(from, to: State; typ, sym, tc: SHORTINT);
...
PROCEDURE NewComment*(from, to: SHORTINT; nested: BOOLEAN);
	VAR com: Comment;

	PROCEDURE MakeStr(gp: SHORTINT; VAR s: ARRAY OF CHAR);
		VAR i, n: SHORTINT; gn: CRT.GraphNode; set: CRT.Set;
	BEGIN
		i := 0;
		WHILE gp # 0 DO
			CRT.GetNode(gp, gn);
...
PROCEDURE ConvertToStates*(gp0, sp: SHORTINT);
(*note: gn.line is abused as a state number!*)
	VAR n: SHORTINT; S: ARRAY maxStates OF State;

	PROCEDURE TheState(gp: SHORTINT): State;
	...
	PROCEDURE Step(from: State; gp: SHORTINT);
	...
	PROCEDURE FindTrans(gp: SHORTINT; state: State);
...
PROCEDURE MatchDFA* (s: ARRAY OF CHAR; sp: SHORTINT; VAR matchedSp: SHORTINT);
	VAR state, to: State; a: Action; i, len: SHORTINT;
BEGIN (*s with quotes*)
	...
	WHILE i < len DO (*make new DFA for s[i..len-1]*)
		to := NewState();
		NewTransition(state, to, CRT.char, SHORT(ORD(s[i])), CRT.normTrans);
		state := to; INC(i)
	END;
...
VAR c: Action; seta, setb, setc: CRT.Set;
	...
	PROCEDURE CombineTransCodes(t1, t2: SHORTINT; VAR result:SHORTINT);
	BEGIN
...
VAR
	...
	PROCEDURE AddMeltedSet(nr: SHORTINT; VAR set: CRT.Set);
	VAR m: Melted;
	BEGIN
		m := firstMelted;
	...
	PROCEDURE GetStateSet(t: Target; VAR set: CRT.Set; VAR endOf: SHORTINT; VAR ctx:BOOLEAN);
	VAR statenr: SHORTINT; (*lastS: State;*)
	BEGIN
...
PROCEDURE PrintSymbol(typ, val, width: SHORTINT);
VAR name: CRT.Name; len: SHORTINT;
BEGIN
...
VAR action: Action; first: BOOLEAN; state: State; i: SHORTINT; targ: Target; set: CRT.Set; name: CRT.Name;
BEGIN
	PutS("$-------- states ---------$");
	state := firstState;
	WHILE state # NIL DO
...
PROCEDURE WriteState(state: State);
	VAR action: Action; first, ctxEnd: BOOLEAN; sn: CRT.SymbolNode; endOf: SHORTINT;
		set: CRT.Set;
BEGIN
	endOf := state.endOf;
...
VAR res, i: SHORTINT; bak: ARRAY 64 OF CHAR;
BEGIN
	i := 0;
	WHILE name[i] # 0X DO
		bak[i] := name[i];	INC(i)
...
VAR
	...
	startTab: ARRAY 128 OF SHORTINT;
	com:      Comment;
	i, j, l:  SHORTINT;
	gn:       CRT.GraphNode;
	sn:       CRT.SymbolNode;
	...
	PROCEDURE FillStartTab;
		VAR action: Action; i, targetState: SHORTINT; class: CRT.Set;
	BEGIN
